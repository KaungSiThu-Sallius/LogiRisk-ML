...
def rf_objective(trial,  X_train, y_train):
    with mlflow.start_run(run_name=f"Random Forest trial {trial.number}", nested=True):
        ...
        cv_scores = cross_val_score(rf_model, X_train, y_train, cv=3, scoring='f1_weighted', n_jobs=-1)
...
def xgb_objective(trial, X_train, y_train):
    with mlflow.start_run(run_name=f"XGB trial {trial.number}", nested=True):
        ...
        cv_scores = cross_val_score(xgb_model, X_train, y_train, cv=5, scoring='f1_weighted', n_jobs=-1)
...
def run_training():
    ...
    with mlflow.start_run(run_name="RF_Optimization"):
        ...
        study.optimize(lambda trial: rf_objective(trial, X_train, y_train), n_trials=10)
    ...
    with mlflow.start_run(run_name="XGB_Optimization"):
        ...
        mlflow.log_metric("test_f1", test_f1)
        mlflow.log_metric("overfit_gap", train_acc- test_acc)
        mlflow.log_metric("overfit_gap_f1", train_f1 - test_f1)
        ...
        plot_importance(best_xgb, ax=ax, max_num_features=20)
    ...
    save_artifacts(best_xgb)
        
        
def save_artifacts(model):
    model_path = os.path.join(os.getcwd(), 'models')
    model.save_model(os.path.join(model_path, 'best_xgb_model_logi_risk.json'))
    print(f"Production artifacts secured in: {model_path}")
